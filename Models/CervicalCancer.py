...
filepaths = []
filepaths.append('/home/pikachu/MLProjects/CervicalCancerScreening/train/Type_1/')
filepaths.append('/home/pikachu/MLProjects/CervicalCancerScreening/train/Type_2/')
filepaths.append('/home/pikachu/MLProjects/CervicalCancerScreening/train/Type_3/')
...
sample_subm = pd.read_csv("~/Documents/MLProjects/CervicalCancerImages/sample_submission.csv")
ids = sample_subm['image_name'].values

for id in ids:
    print('Predict for image {}'.format(id))
    files = glob.glob("~/Documents/MLProjects/CervicalCancerImages/test/" + id)
    image_list = []
    for f in files:
        image = cv2.imread(f)
        image = cv2.resize(image, conf['image_shape'])
